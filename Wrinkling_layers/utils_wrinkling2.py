...
def mean_std(data):    
    ...
    for geom in range(len(data)):
        ...
        gt_mean = gt_mean + torch.mean(gt, dim = -2, keepdim=True)
...
def load_graph(path=None, data_types=None, steps=101, split =None ,shuffle = True, comb_params = None):
    os.chdir(path)
    ...
    for geomind in range(num_files):
        ...
        Ei = torch.zeros_like(node_type)
        Ei = torch.where(node_type==0, 200.0, comb_params[geomind][0])
        
        nui = torch.zeros_like(node_type)
        nui = torch.where(node_type==0, comb_params[geomind][1], 0.3)
         
        # Concatenate node features without BCs
        node_features = torch.cat([coords, Ei, nui], dim=-1)       
        
        # Concatenate fields as ground truth        
        field = torch.cat([field1[-1,:,:], field2[-1,:,:]], dim=-1)        
